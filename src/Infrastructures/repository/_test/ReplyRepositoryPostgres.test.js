...
describe('ReplyRepositoryPostgres', () => {
    beforeAll(async () => {
        ...
        await ThreadTableTestHelper.postThread({id: TEST_DATA.threadId, owner: TEST_DATA.owner});
        await CommentsTableTestHelper.addComment({id: TEST_DATA.commentId, owner: TEST_DATA.owner});
    });
    ...
    describe('add reply', () => {
        it('should persist reply and return reply correctly', async () => {
            ...
            const reply = await commentRepositoryPostgres.addReply(replyPayload, TEST_DATA.commentId, TEST_DATA.owner);
    ...
    describe('verify reply owner', () => {
        it('should throw error when reply not found', async () => {
            ...
            await expect(replyRepositoryPostgres.verifyReplyOwner('comment-321', TEST_DATA.owner)).rejects.toThrowError('VERIFY_REPLY.NOT_FOUND');
        });
        ...
        it('should throw error when owner not match', async () => {
            ...
            await expect(replyRepositoryPostgres.verifyReplyOwner(TEST_DATA.id, 'user-321')).rejects.toThrowError('VERIFY_REPLY.OWNER_NOT_MATCH');
        });
    });
    ...
    describe('get replies by comment id', () => {
        it('should return replies correctly', async () => {
            ...
            expect(replies).toHaveLength(1);
            expect(replies[0].id).toStrictEqual(TEST_DATA.id);
        });
    });
